            ...
            if bar_addr == new_base {
                ...
                if let Some(mut shm_regions) = virtio_dev.get_shm_regions() {
                    if shm_regions.addr.raw_value() == old_base {
                        ...
                        let mut mem_region =
                            hypervisor::kvm_bindings::kvm_userspace_memory_region {
...
impl DeviceManager {
    ...
    fn create_kvm_device(vm: &Arc<dyn hypervisor::Vm>) -> DeviceManagerResult<DeviceFd> {
        let mut vfio_dev = hypervisor::kvm_bindings::kvm_create_device {
            type_: hypervisor::kvm_bindings::kvm_device_type_KVM_DEV_TYPE_VFIO,
            fd: 0,
            flags: 0,
        };
