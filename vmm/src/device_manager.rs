...
impl DeviceManager {
    ...
    fn make_virtio_pmem_device(
        ...
        let (region_base, region_size) = if let Some((base, size)) = region_range {
            ...
            // `phys_bits`.
            //
            // When the device-memory region is full (e.g. entirely
            // consumed by virtio-mem zones) or absent, fall back to
            // the per-segment 64-bit MMIO window.
            let mm = self.memory_manager.lock().unwrap();
            let base = mm
                .device_memory()
                .and_then(|dm| {
                    dm.allocator().lock().unwrap().allocate_first_fit(
                        None,
                        size as GuestUsize,
                        Some(0x0020_0000),
                    )
