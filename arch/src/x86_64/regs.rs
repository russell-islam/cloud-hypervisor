...
pub enum Error {
    ...
    GetStatusRegisters(anyhow::Error),
    /// Failed to set base registers for this CPU.
    SetBaseRegisters(anyhow::Error),
    /// Failed to configure the FPU.
    SetFPURegisters(anyhow::Error),
    /// Setting up MSRs failed.
    SetModelSpecificRegisters(anyhow::Error),
    /// Failed to set SREGs for this CPU.
    SetStatusRegisters(anyhow::Error),
...
pub fn setup_fpu(vcpu: &Arc<dyn hypervisor::Vcpu>) -> Result<()> {
    ...
    vcpu.set_fpu(&fpu)
        .map_err(|e| Error::SetFPURegisters(e.into()))
}
...
pub fn setup_msrs(vcpu: &Arc<dyn hypervisor::Vcpu>) -> Result<()> {
    vcpu.set_msrs(&boot_msr_entries())
        .map_err(|e| Error::SetModelSpecificRegisters(e.into()))?;
...
pub fn setup_regs(
    ...
    vcpu.set_regs(&regs)
        .map_err(|e| Error::SetBaseRegisters(e.into()))
}
...
pub fn setup_sregs(
    ...
    let mut sregs: kvm_sregs = vcpu
        .get_sregs()
        .map_err(|e| Error::GetStatusRegisters(e.into()))?;
    ...
    vcpu.set_sregs(&sregs)
        .map_err(|e| Error::SetStatusRegisters(e.into()))
}
