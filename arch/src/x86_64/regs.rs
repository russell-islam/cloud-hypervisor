...
use crate::hypervisor::bindings::{FpuState, SpecialRegisters, StandardRegisters};
...
pub fn setup_fpu(vcpu: &Arc<dyn GenVcpuFd>) -> Result<()> {
...
///
/// * `vcpu` - Structure for the VCPU that holds the VCPU's fd.
pub fn setup_msrs(vcpu: &Arc<dyn GenVcpuFd>) -> Result<()> {
